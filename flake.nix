{
  description = "jshvn-dispatch: starts my own repos' workflows on UTC slots";
    ...
    in
    {
      packages = forAll (pkgs: {
        default = pkgs.buildGoModule {
          ...
          postInstall = "mv $out/bin/dispatch $out/bin/jshvn-dispatch";
          meta.mainProgram = "jshvn-dispatch";
        };
      });
      ...
      checks = forAll (
        ...
        in
        {
          ...
          module = pkgs.runCommand "jshvn-dispatch-units" { } ''
            service=${units."jshvn-dispatch.service".unit}/jshvn-dispatch.service
            exe=$(sed -n 's/^ExecStart=//p' "$service")
            test -x "$exe" || { echo "ExecStart $exe is not an executable" >&2; exit 1; }
            install -Dm644 -t $out "$service" ${units."jshvn-dispatch.timer".unit}/jshvn-dispatch.timer
          '';
        }
      );
