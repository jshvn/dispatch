...
- `src/targets.ts` -- which repo and workflow each expression means. Routine changes touch
  only this file, plus the matching cron in `wrangler.jsonc`.
- `src/github.ts` -- App JWT, installation token, `workflow_dispatch`. No SDK, two requests.
- `src/index.ts` -- `scheduled` creates one instance per firing. The instance dispatches
  every target claiming that cron, then ends.
- `test/dispatch.test.ts` -- the units. `test/index.test.ts` -- the handlers, with
  `src/github` mocked.
...
Accepted ceilings: no backfill, no outcome monitoring, the crons duplicated across two
files, and a subrequest budget that bounds how many targets one expression can carry.

## Must knows

Each of these fails silently, or only in production.

- **The crons live in two files.** `triggers.crons` in `wrangler.jsonc`, `cron` in
  `src/targets.ts`. A target that is not also a trigger never runs. A trigger no target
  claims throws every time it fires. The tests assert both ways.
...
- **5 cron expressions per Cloudflare account** on the free plan, shared by every Worker on
  the account. Targets sharing an expression share one trigger.
...
`task check` is everything CI runs: typecheck, format, tests, dry-run deploy.
...
- **A PEM private-key header in Bash tool text trips the secret-scan hook**, test fixtures
  included. `test/dispatch.test.ts` builds the header from fragments. Write such files with
  Write or Edit.
